...
public class TestDicomMismatch {
    ...
    public static void displayAllRelevantInformationForInvestigation(){
        ...
        for(int i=0;i<GeneralUtils.getSpecimenListMRI().length;i++){strTab[1+2*i][0]=GeneralUtils.getSpecimenListMRI()[i]+"_T1";strTab[1+2*i+1][0]=GeneralUtils.getSpecimenListMRI()[i]+"_T2";};
        for(int j=0;j<GeneralUtils.getParams().length;j++)strTab[0][1+j]=paramsTranslation[j];


        for(int i=0;i<GeneralUtils.getSpecimenListMRI().length;i++){
            String specimen=GeneralUtils.getSpecimenListMRI()[i];
            for(String mod : GeneralUtils.getModalities()){
